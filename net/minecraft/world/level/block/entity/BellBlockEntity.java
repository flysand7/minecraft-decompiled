...
public class BellBlockEntity extends BlockEntity {
   ...
   private void updateEntities() {
      ...
      if (!this.level.isClientSide) {
         ...
         while(var4.hasNext()) {
            ...
            if (var3.isAlive() && !var3.isRemoved() && var1.closerToCenterThan(var3.position(), 32.0D)) {
   ...
   private static boolean areRaidersNearby(BlockPos var0, List<LivingEntity> var1) {
      ...
      } while(!var3.isAlive() || var3.isRemoved() || !var0.closerToCenterThan(var3.position(), 32.0D) || !var3.getType().is(EntityTypeTags.RAIDERS));
   ...
   private static void showBellParticles(Level var0, BlockPos var1, List<LivingEntity> var2) {
      ...
      int var4 = (int)var2.stream().filter((var1x) -> {
         return var1.closerToCenterThan(var1x.position(), 48.0D);
      }).count();
      var2.stream().filter((var1x) -> {
         return isRaiderWithinRange(var1, var1x);
      }).forEach((var4x) -> {
   ...
   private static boolean isRaiderWithinRange(BlockPos var0, LivingEntity var1) {
      return var1.isAlive() && !var1.isRemoved() && var0.closerToCenterThan(var1.position(), 48.0D) && var1.getType().is(EntityTypeTags.RAIDERS);
   }
