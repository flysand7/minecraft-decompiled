...
public abstract class Display extends Entity {
   ...
   protected void readAdditionalSaveData(CompoundTag var1) {
      ...
      if (var1.contains("start_interpolation", 99)) {
         ...
         this.setInterpolationDelay(var2);
      }
      ...
      if (var1.contains("billboard", 8)) {
         ...
         Objects.requireNonNull(var10002);
         var10000.resultOrPartial(Util.prefix("Display entity", var10002::error)).ifPresent((var1x) -> {
            this.setBillboardConstraints((Display.BillboardConstraints)var1x.getFirst());
         });
      }
   ...
   protected void addAdditionalSaveData(CompoundTag var1) {
      Transformation.EXTENDED_CODEC.encodeStart(NbtOps.INSTANCE, createTransformation(this.entityData)).result().ifPresent((var1x) -> {
         var1.put("transformation", var1x);
      });
      Display.BillboardConstraints.CODEC.encodeStart(NbtOps.INSTANCE, this.getBillboardConstraints()).result().ifPresent((var1x) -> {
      ...
      if (var2 != null) {
         Brightness.CODEC.encodeStart(NbtOps.INSTANCE, var2).result().ifPresent((var1x) -> {
   ...
   private int getInterpolationDuration() {
      return (Integer)this.entityData.get(DATA_INTERPOLATION_DURATION_ID);
   }
   ...
   private int getInterpolationDelay() {
      return (Integer)this.entityData.get(DATA_INTERPOLATION_START_DELTA_TICKS_ID);
   }
   ...
   static {
      DATA_INTERPOLATION_START_DELTA_TICKS_ID = SynchedEntityData.defineId(Display.class, EntityDataSerializers.INT);
      DATA_INTERPOLATION_DURATION_ID = SynchedEntityData.defineId(Display.class, EntityDataSerializers.INT);
      ...
      RENDER_STATE_IDS = IntSet.of(new int[]{DATA_TRANSLATION_ID.getId(), DATA_SCALE_ID.getId(), DATA_LEFT_ROTATION_ID.getId(), DATA_RIGHT_ROTATION_ID.getId(), DATA_BILLBOARD_RENDER_CONSTRAINTS_ID.getId(), DATA_BRIGHTNESS_OVERRIDE_ID.getId(), DATA_SHADOW_RADIUS_ID.getId(), DATA_SHADOW_STRENGTH_ID.getId()});
   }
   ...
   public static record RenderState(Display.GenericInterpolator<Transformation> a, Display.BillboardConstraints b, int c, Display.FloatInterpolator d, Display.FloatInterpolator e, int f) {
      ...
      public RenderState(Display.GenericInterpolator<Transformation> var1, Display.BillboardConstraints var2, int var3, Display.FloatInterpolator var4, Display.FloatInterpolator var5, int var6) {
   ...
   public static class TextDisplay extends Display {
      ...
      protected void defineSynchedData() {
         ...
         this.entityData.define(DATA_TEXT_OPACITY_ID, -1);
      ...
      protected void readAdditionalSaveData(CompoundTag var1) {
         ...
         if (var3.isPresent()) {
            byte var9;
            switch((Display.TextDisplay.Align)var3.get()) {
            case CENTER:
               var9 = var2;
               break;
            case LEFT:
      ...
      protected void addAdditionalSaveData(CompoundTag var1) {
         ...
         Display.TextDisplay.Align.CODEC.encodeStart(NbtOps.INSTANCE, getAlign(var2)).result().ifPresent((var1x) -> {
      ...
      static {
         ...
         TEXT_RENDER_STATE_IDS = IntSet.of(new int[]{DATA_TEXT_ID.getId(), DATA_LINE_WIDTH_ID.getId(), DATA_BACKGROUND_COLOR_ID.getId(), DATA_TEXT_OPACITY_ID.getId(), DATA_STYLE_FLAGS_ID.getId()});
      }
   ...
   public static class ItemDisplay extends Display {
      ...
      protected void addAdditionalSaveData(CompoundTag var1) {
         ...
         ItemDisplayContext.CODEC.encodeStart(NbtOps.INSTANCE, this.getItemTransform()).result().ifPresent((var1x) -> {
