...
public class AABB {
   ...
   public double distanceToSqr(Vec3 var1) {
      double var2 = Math.max(Math.max(this.minX - var1.x, var1.x - this.maxX), 0.0D);
      double var4 = Math.max(Math.max(this.minY - var1.y, var1.y - this.maxY), 0.0D);
      double var6 = Math.max(Math.max(this.minZ - var1.z, var1.z - this.maxZ), 0.0D);
