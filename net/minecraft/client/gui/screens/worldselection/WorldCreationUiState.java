...
public class WorldCreationUiState {
   ...
   public WorldCreationUiState(Path var1, WorldCreationContext var2, Optional<ResourceKey<WorldPreset>> var3, OptionalLong var4) {
      ...
      this.settings = var2;
      this.worldType = new WorldCreationUiState.WorldTypeEntry((Holder)findPreset(var2, var3).orElse((Object)null));
   ...
   private void updatePresetLists() {
      ...
      this.normalPresetList.addAll((Collection)getNonEmptyList(var1, WorldPresetTags.NORMAL).orElseGet(() -> {
         return var1.holders().map(WorldCreationUiState.WorldTypeEntry::new).toList();
      }));
      ...
      if (var2 != null) {
         this.worldType = (WorldCreationUiState.WorldTypeEntry)findPreset(this.getSettings(), var2.unwrapKey()).map(WorldCreationUiState.WorldTypeEntry::new).orElse((WorldCreationUiState.WorldTypeEntry)this.normalPresetList.get(0));
      }
   ...
   private static Optional<List<WorldCreationUiState.WorldTypeEntry>> getNonEmptyList(Registry<WorldPreset> var0, TagKey<WorldPreset> var1) {
      return var0.getTag(var1).map((var0x) -> {
         return var0x.stream().map(WorldCreationUiState.WorldTypeEntry::new).toList();
      }).filter((var0x) -> {
