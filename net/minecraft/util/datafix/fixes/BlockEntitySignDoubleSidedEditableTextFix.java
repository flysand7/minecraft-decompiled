...
public class BlockEntitySignDoubleSidedEditableTextFix extends NamedEntityFix {
   ...
   private static <T> Dynamic<T> getTextList(Dynamic<T> var0, String var1) {
      ...
      return var0.createList(Stream.of((Dynamic)var0.get(var1 + "1").result().orElse(var2), (Dynamic)var0.get(var1 + "2").result().orElse(var2), (Dynamic)var0.get(var1 + "3").result().orElse(var2), (Dynamic)var0.get(var1 + "4").result().orElse(var2)));
   }
