...
public record InclusiveRange<T extends Comparable<T>>(T b, T c) {
   ...
   public static <T extends Comparable<T>> Codec<InclusiveRange<T>> codec(Codec<T> var0, T var1, T var2) {
      return ExtraCodecs.validate(codec(var0), (var2x) -> {
         if (var2x.minInclusive().compareTo(var1) < 0) {
            return DataResult.error(() -> {
               return "Range limit too low, expected at least " + var1 + " [" + var2x.minInclusive() + "-" + var2x.maxInclusive() + "]";
            });
         } else {
            return var2x.maxInclusive().compareTo(var2) > 0 ? DataResult.error(() -> {
               return "Range limit too high, expected at most " + var2 + " [" + var2x.minInclusive() + "-" + var2x.maxInclusive() + "]";
