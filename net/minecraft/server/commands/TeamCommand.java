...
public class TeamCommand {
   ...
   private static int leaveTeam(CommandSourceStack var0, Collection<String> var1) {
      ...
      if (var1.size() == 1) {
         var0.sendSuccess(() -> {
            return Component.translatable("commands.team.leave.success.single", var1.iterator().next());
         }, true);
      } else {
         var0.sendSuccess(() -> {
   ...
   private static int joinTeam(CommandSourceStack var0, PlayerTeam var1, Collection<String> var2) {
      ...
      if (var2.size() == 1) {
         var0.sendSuccess(() -> {
            return Component.translatable("commands.team.join.success.single", var2.iterator().next(), var1.getFormattedDisplayName());
         }, true);
      } else {
         var0.sendSuccess(() -> {
