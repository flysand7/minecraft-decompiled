...
public class GiveCommand {
   ...
   private static int giveItem(CommandSourceStack var0, ItemInput var1, Collection<ServerPlayer> var2, int var3) throws CommandSyntaxException {
      ...
      if (var3 > var5) {
         ...
         if (var2.size() == 1) {
            var0.sendSuccess(() -> {
               return Component.translatable("commands.give.success.single", var3, var6.getDisplayName(), ((ServerPlayer)var2.iterator().next()).getDisplayName());
            }, true);
         } else {
            var0.sendSuccess(() -> {
               return Component.translatable("commands.give.success.single", var3, var6.getDisplayName(), var2.size());
            }, true);
         }
